...
(ert-deftest hym-workspace-sidebar-opens-everywhere-at-startup ()
  (let ((hym-workspace-sidebar--visible nil)
        requested-state)
    (cl-letf (((symbol-function 'hym-workspace-sidebar--set-all-tabs-visible)
               (lambda (visible) (setq requested-state visible))))
      (hym-workspace-sidebar--open-at-startup))
    (should hym-workspace-sidebar--visible)
    (should requested-state)))

(ert-deftest hym-workspace-sidebar-toggle-applies-to-all-tabs-and-groups ()
  (hym-workspace-sidebar-test-with-registry
    (let ((saved-tabs (frame-parameter nil 'tabs))
          (saved-mode hym-tabs-mode)
          (hym/tab-group-last-tab (make-hash-table :test 'equal))
          (hym-workspace-sidebar--visible nil))
      (unwind-protect
          (progn
            (when hym-tabs-mode (hym-tabs-mode -1))
            (set-frame-parameter nil 'tabs nil)
            (hym-tabs-mode 1)
            (tab-bar-rename-tab "general")
            (tab-bar-change-tab-group hym/default-tab-group)
            (tab-bar-new-tab)
            (tab-bar-rename-tab "alpha")
            (tab-bar-change-tab-group "one")
            (tab-bar-new-tab)
            (tab-bar-rename-tab "beta")
            (tab-bar-change-tab-group "two")
            (let ((original-tab (1+ (tab-bar--current-tab-index))))
              (hym-workspace-sidebar-toggle)
              (should (= (1+ (tab-bar--current-tab-index)) original-tab))
              (dotimes (index 3)
                (let ((hym/tab-restore-group-selection nil))
                  (tab-bar-select-tab (1+ index)))
                (should (get-buffer-window hym-workspace-sidebar-buffer-name)))
              (hym-workspace-sidebar-toggle)
              (should-not hym-workspace-sidebar--visible)
              (dotimes (index 3)
                (let ((hym/tab-restore-group-selection nil))
                  (tab-bar-select-tab (1+ index)))
                (should-not
                 (get-buffer-window hym-workspace-sidebar-buffer-name)))))
        (setq hym-workspace-sidebar--visible nil)
        (hym-workspace-sidebar--set-all-tabs-visible nil)
