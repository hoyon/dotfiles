...
local function shutdown_menu()
    local grabber = awful.keygrabber.run(function(mod, key, event)
        ...
        elseif key == "l" then awesome.quit()
        end
        keygrabber.stop()
    end)
end
...
local media_widget = require("media")
...
globalkeys = awful.util.table.join(
    ...
    awful.key({ modkey,           }, "Tab", function () awful.screen.focus_relative(1) end,
              {description = "switch monitor", group = "client"}),

    -- Standard program
    awful.key({ modkey,           }, "Return",
                function()
                    awful.spawn(awful.util.get_configuration_dir() .. "openterm.sh", false)
                end,
    ...
    -----------------------------
    -- Media keys
    awful.key({ fnkey }, "d", function() volume_widget:up() end,
              {description = "volume up", group = "media"}),
    awful.key({ fnkey }, "s", function() volume_widget:down() end,
              {description = "volume down", group = "media"}),
    awful.key({ fnkey }, "f", function() volume_widget:mute() end,
              {description = "volume mute", group = "media"}),
    awful.key({ fnkey }, "w", function() media_widget:toggle() end,
              {description = "toggle media play/pause", group = "media"}),
    awful.key({ fnkey }, "q", function() media_widget:prev() end,
...
-- Remove borders when single window
